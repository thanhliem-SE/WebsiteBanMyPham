 ...
 * .password(passwordEncoder().encode("user2Pass")).roles("USER").and().withUser
 * ("admin") .password(passwordEncoder().encode("adminPass")).roles("ADMIN"); }
 * 
 * @Override protected void configure(HttpSecurity http) throws Exception {
 * http.authorizeRequests() // .antMatchers("/user/**").hasRole("USER")
 * .antMatchers("/admin/**").hasRole("ADMIN").and().formLogin().loginPage(
 * "/showFormLogin").loginProcessingUrl("/login").defaultSuccessUrl("/").and().
 * logout().permitAll();
